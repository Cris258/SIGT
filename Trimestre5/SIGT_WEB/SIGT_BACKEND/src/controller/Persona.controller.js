...
export const createPersona = async (req, res) => {
  try {
    ...
    const passwordHash = await bcrypt.hash(dataPersona.Password, salt);
    const createPersona = await personaModel.create({
      ...
      Rol_FK: parseInt(dataPersona.Rol_FK),
      EstadoPersona_FK: 1,
    });
  ...
  try {
    ...
    return res.status(201).json({
      ...
      persona: {
        ...
        NumeroDocumento: newPersona.NumeroDocumento,
        TipoDocumento: newPersona.TipoDocumento,
        Primer_Nombre: newPersona.Primer_Nombre,
        Segundo_Nombre: newPersona.Segundo_Nombre,
        Primer_Apellido: newPersona.Primer_Apellido,
        Segundo_Apellido: newPersona.Segundo_Apellido,
        Telefono: newPersona.Telefono,
        Correo: newPersona.Correo,
        Rol_FK: newPersona.Rol_FK,
      },
    });
  } catch (error) {
    return res.status(500).json({
