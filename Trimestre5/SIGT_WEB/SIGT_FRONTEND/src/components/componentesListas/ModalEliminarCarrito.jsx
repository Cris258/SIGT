...
export default function ModalEliminarCarrito({ carrito, onClose, onConfirmar }) {
  ...
  const handleEliminar = async () => {
    setDebugMsg("handleEliminar() invocado"); // feedback inmediato en UI
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (typeof onConfirmar === "function") {
          try {
            onConfirmar(carrito); // preferimos pasar objeto
          } catch (err) {
            console.warn("onConfirmar falló pasando objeto, intentando con id", err);
            try { onConfirmar(carrito.idCarrito); } catch (e) { console.error(e); }
          }
        } else {
          console.warn("onConfirmar no es función.");
        }

        try { onClose(); } catch (e) { console.warn("onClose lanzó error:", e); }
